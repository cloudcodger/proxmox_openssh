...
Called on inventory hosts that have Proxmox VE installed on them and optionally uses the `proxmoxer` Python module and its `openssh` backend to connect to one of the hosts to configure the user, API token and permissions ACLs.

This role will install the `prometheus-pve-exporter` Python module in a virtual environment and configure it to run as a service that will provide metrics to either Grafana Prometheus or Grafana Mimir.
